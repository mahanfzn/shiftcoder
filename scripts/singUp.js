...
function eventListeaner() {
    ...
    validation[4].addEventListener('blur', sendData)//reapetThePass
...
function ValidTheName(name) {
    if (name.target.value !== '' && name.target.value.length >= 3) {
        ...
        send.sendMessage('لطفا اسم خود را صحیح وارد کنید', 'aler alert-danger text-center', 'meesageName')
...
function ValidTheeamil(getEmail) {
    ...
    if (getEmail.target.value.match(mailformat) && getEmail.target.value !== '') {
        ...
        validation[5].classList.add('error')
        send.sendMessage('لطفا ایمیل خود را صحیح وارد کنید', 'aler alert-danger text-center', 'meesageEmail')
    }
}
 // valid the phone
function ValidThePhone(getPhone) {
    ...
    if (getPhone.target.value.match(phoneFormat)) {
        ...
        validation[5].classList.add('error')
        send.sendMessage('لطفا شماره خود را صحیح وارد کنید', 'aler alert-danger text-center', 'meesagePhone')
...
function ValidThePassword(pass) {
    if (pass.target.value !== '' && pass.target.value.length >= 8) {
        ...
        validation[5].classList.add('error')
        send.sendMessage('لطفا رمز خود را وارد کنید', 'aler alert-danger text-center', 'meesagePassword')
...
function ValidTheReapetPass(pass, reapPass) {
    if (pass === reapPass) {
        ...
        validation[5].classList.add('error')
        send.sendMessage('تکرارگذرواژه با گذرواژه بابر نیست', 'aler alert-danger text-center', 'meesageRepass')
